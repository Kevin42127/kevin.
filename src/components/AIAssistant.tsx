                    ...
                    {message.role === 'assistant' ? (() => {
                      const formatText = (text: string) => {
                        ...
                        formatted = formatted.replace(/\[DOWNLOAD_RESUME_ZH\]/g, 
                          '<a href="/陳梓敬_AI工程師_履歷.pdf" download="陳梓敬_AI工程師_履歷.pdf" class="inline-flex items-center justify-center w-10 h-10 bg-[var(--color-primary)] text-white rounded-full hover:bg-[var(--color-primary-dark)] transition-colors shadow-md hover:shadow-lg" style="text-decoration: none;" title="' + (currentLanguage === 'en' ? 'Download Resume' : '下載履歷') + '"><span class="material-symbols-outlined text-xl">download</span></a>')
                        
                        formatted = formatted.replace(/\[VIEW_PORTFOLIO\]/g, 
                          '<button onclick="document.getElementById(\'portfolio\')?.scrollIntoView({behavior: \'smooth\'})" class="inline-flex items-center justify-center w-10 h-10 bg-[var(--color-primary)] text-white rounded-full hover:bg-[var(--color-primary-dark)] transition-colors shadow-md hover:shadow-lg" title="' + (currentLanguage === 'en' ? 'View Portfolio' : '查看作品集') + '"><span class="material-symbols-outlined text-xl">work</span></button>')
                        
                        formatted = formatted.replace(/\[CONTACT_FORM\]/g, 
                          '<button onclick="document.getElementById(\'contact\')?.scrollIntoView({behavior: \'smooth\'})" class="inline-flex items-center justify-center w-10 h-10 bg-[var(--color-primary)] text-white rounded-full hover:bg-[var(--color-primary-dark)] transition-colors shadow-md hover:shadow-lg" title="' + (currentLanguage === 'en' ? 'Contact Form' : '聯繫表單') + '"><span class="material-symbols-outlined text-xl">mail</span></button>')
